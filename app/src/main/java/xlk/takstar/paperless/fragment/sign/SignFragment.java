...
public class SignFragment extends BaseFragment<SignPresenter> implements SignContract.View {
    ...
    @Override
    public void updateView(List<InterfaceRoom.pbui_Item_MeetRoomDevSeatDetailInfo> itemList, boolean isShow) {
        if (getActivity() == null) {
            return;
        }
...
//                RelativeLayout.LayoutParams.WRAP_CONTENT, RelativeLayout.LayoutParams.WRAP_CONTENT);
        ...
        if (!TextUtils.isEmpty(memberName)) {
            ...
            item_seat_member.setTextColor((item.getIssignin() == 1)
                    ? MyApplication.appContext.getColor(R.color.signed_text_color)
                    : MyApplication.appContext.getColor(R.color.unsigned_text_color)
            );
        } else {
